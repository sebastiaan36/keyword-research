...
class GetDataController extends Controller
{
    public function GetData()
    {
        ...
        //get keywords not yet processed
        $keywords = Keywords::where('keyword_spell', null)->pluck('keyword');
        if($keywords->count() === 0){
            return redirect()->route('dashboard')->with('status', 'Geen keywords om op te halen');
        }
        // Instead of 'login' and 'password' use your credentials from https://app.dataforseo.com/api-dashboard
        $client = new RestClient($api_url, null, env('SEOUSER'), env('SEOPASS'));
        ...
        try {
            ...
            foreach($result['tasks'][0]['result'] as $keyword){
                $k = Keywords::where('keyword', $keyword['keyword'])->first();
                if(isset($k)){
                if($keyword['spell'] === null)
                    $k->keyword_spell = $keyword['keyword'];
                else{
                    $k->keyword_spell = $keyword['spell'];
                }
                $k->search_volume = $keyword['search_volume'];
                $k->competition = $keyword['competition'];
                $k->competition_index = $keyword['competition_index'];
                $k->cpc_high = $keyword['high_top_of_page_bid'];
                $k->cpc_low = $keyword['low_top_of_page_bid'];
                $k->historical = $keyword['monthly_searches'];
        ...
        $client = null;
        return redirect()->route('dashboard')->with('status', $keywords->count() . ' keywords opgehaald');
    }
}
class RestClient {
